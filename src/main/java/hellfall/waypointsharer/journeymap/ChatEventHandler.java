...
public class ChatEventHandler {
    ...
    public void handleChatEvent(ClientChatReceivedEvent event) {
        ...
        if (key != null) {
            ...
            IChatComponent component = new ChatComponentTranslation("waypointsharer.sharemessage", player, name.substring(0, name.indexOf(delimiter)));
            text = text.substring(text.indexOf(key));
            component.getChatStyle().setChatClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/ws-jm-add-waypoint " + text));
